...
int main(int argc, char** argv)
{
  ...
  while((type=readline(ifile,&context))!=END)
  {
    ...
    Cycle **cycle_array = malloc(sizeof(Cycle*) * num_edge); // we cannot have more than num_edge cycles
    int num_cycles = 0;
    get_cycles(cycle_array, node_array, edge_array, num_node, num_edge, &num_cycles);
